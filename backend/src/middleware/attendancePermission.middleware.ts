...
export const requireAttendancePermission = (permission: string, allowSelfOnly = false) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // For self-only operations (like marking attendance), check if it's their own record
      if (allowSelfOnly && (req.method === 'POST' || req.method === 'PUT')) {
        const attendanceId = req.params.id;
        const employeeIdFromBody = req.body.employee;
        
        // For new records, check if they're marking for themselves
        if (req.method === 'POST' && employeeIdFromBody) {
          if (employeeIdFromBody !== employee._id.toString()) {
            return res.status(403).json({ 
        ...
        // For updates, check if it's their own attendance record
        if (req.method === 'PUT' && attendanceId) {
          const attendance = await Attendance.findById(attendanceId);
          if (attendance && attendance.employee.toString() !== employee._id.toString()) {
            return res.status(403).json({ 
...
export const requireManagerPermission = (permission: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user) return res.status(401).json({ message: 'Authentication required' });
      ...
      // Check if user is a manager (has manager-level permissions)
      const Employee = (await import('../models/Employee')).default;
      const Department = (await import('../models/Department')).default;
      ...
      // Check if user has manager role or manager-level permissions
