...
export const taxManagementAPI = {
  ...
  calculateTDS: async (amount: number, rate: number) => api.post('/tax-management/calculate-tds', { amount, rate }),
  calculateIncomeTax: async (income: number, deductions?: number) => api.post('/tax-management/calculate-income-tax', { income, deductions }),
};
...
export const agingAnalysisAPI = {
  ...
  exportReport: async (type: 'receivables' | 'payables') => api.get(`/aging-analysis/export/${type}`, { responseType: 'blob' }),
};
