...
export default function BackupLogsManager() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(`/api/backup/logs?${params}`, {
        headers: {
  ...
  const downloadBackup = async (backupId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `backup-${backupId}-${Date.now()}.zip`;
        a.click();
                  ...
                  {log.status === 'success' && (
                    <>
                      ...
                      <Button 
                        variant="outline" 
                        size="sm"
                        onClick={() => downloadBackup(log.backupId)}
                      >
